...
int main(int argc, char** argv){
    int pid;           /* per il valore di returnValue della fork */
    int pidFiglio;      /* per il valore di returnValue della wait */
    ...
    char byte;          /* per controllare la lettura da pipe */
    char ok;            /* per il permesso di procedere, il suo valore non ha importanza */
    ...
    if (piped == NULL){    /* controllo il valore di returnValue della malloc */
        printf("Errore nella malloc di pipe_t.\n");
        exit(5);
    }
    ...
    for(i = 0; i < Q; i++){
        ...
        if((pid = fork()) < 0){
            /* la fork() ha fallito, stampo un messaggio d'errore e returnValue un valore intero d'errore */
        ...
        /* codice processo figlio */
            ...
            for(k = 0; k <= Q; k++){
                if(k != i){
                    close(piped[k][0]);     /* lato di lettura del figlio corrente */
                }
                
                if(k != (i+1)){
                    close(piped[k][1]); /* lato di scrittura del figlio successivo */
                }
            }
            ...
            while((read(F, &cc, 1)) > 0){   /* scorro i caratteri del file */
                ...
                if(cc == '\n'){     /* se arrivo a fine linea */
                	...
                	byte = read(piped[i][0], &ok, 1);   /* ricevo l'ok dal figlio precedente per scrivere */

        			/* controllo il risultato della lettura da pipe */
                    if (byte != 1){
                        printf("Il figlio di indice %d ha scritto un numero di byte sbagliati %d\n", i, byte);
                        exit(-1);
                    }

					/* scrivo su standard output l'indice e il pid del processo, il numero di occorrenze del carattere associato nella linea corrente e il carattere */
                    printf("Figlio con indice %d e pid %d ha letto %d caratteri %c nella linea corrente.\n", i, getpid(), count, argv[i+3][0]);
					
                    /* mando l'ok al filgio successivo tramite pipe */
        			byte = write(piped[i+1][1], &ok, 1);
        			
                    /* controllo il risultato della scrittura su pipe */
                    if (byte != 1){
                        printf("Il figlio di indice %d ha scritto un numero di byte sbagliati %d\n", i, byte);
                        exit(-1);
                    }

					returnValue = count;    /* salvo il valore per ritornarlo */
				    count = 0;              /* azzero il contatore */
                }
            }

            exit(returnValue);    /* restituisco il numero di occorrenze del carattere nell'ultima linea */
        }
    }

    /* codice processo padre */

    /* il padre chiude tutte le pipe che non usa, eccetto la prima e l'ultima */
    for(k = 0; k <= Q; k++){
        if(k != 0){
            close(piped[k][1]); /* primo lato di scrittura */
        }
        if(k != Q){
            close(piped[k][0]); /* ultimo lato di lettura */
        }
    }

    for(i = 1; i <= L; i++){
        printf("\nLinea %d del file %s\n", i, argv[1]); /* scrivo su standard output la presentazioni della linea */

        byte = write(piped[0][1], &ok, 1);  /* mando l'ok al primo figlio di indice 0 */
		...
		byte = read(piped[Q][0], &ok, 1);   /* ricevo l'ok dall'ultimo figlio per far ripartire il ring */
        
        /* controllo il risultato della lettura da pipe */
        if (byte != 1){
            printf("Padre ha letto un numero di byte sbagliati %d\n", byte);
            /* si aspettano comunque i figli */
        }
    }
